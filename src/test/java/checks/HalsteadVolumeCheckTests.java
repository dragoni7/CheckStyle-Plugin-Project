...
public class HalsteadVolumeCheckTests {
	@Test
	public void finishTreeTest() {
		DetailAST mockAST = mock(DetailAST.class);
		doReturn(1).when(mockAST).getLineNo();
		HalsteadVolumeCheck check1 = spy(new HalsteadVolumeCheck());
		...
		HalsteadVolumeCheck check2 = spy(new HalsteadVolumeCheck());
		...
		verify(check2, times(1)).log(anyInt(), anyString()); // Should log the volume
